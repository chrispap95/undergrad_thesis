...
int main(int argc, char* argv[]) {	// Begin main program.
	...
	for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
		...
		for (Int_t ip = 0; ip < np; ip++) {    //Find the first J/psi
			...
			if (part->GetPdgCode() == 443) iJpsi = ip;
			if (part->GetPdgCode() == 22 && part->GetStatusCode() > 0) iGamma.push_back(ip);
		}
		if (iJpsi == 0) {
			--iEvent;
			continue;
		...
		for (Int_t i = 0; i < iGamma.size(); ++i) {
			TParticle* gamma = (TParticle*)particles->At(iGamma[i]);
